...
public class IntegerCalc implements Type<Integer> {
    private final boolean flag;

    public IntegerCalc(boolean flag) {
        this.flag = flag;
    }

    @Override
    public Integer add(Integer x, Integer y) {
        if (flag && ((x > 0 && y > 0 && Integer.MAX_VALUE - x < y)
    ...
    public Integer subtract(Integer x, Integer y) {
        if (flag && ((x == 0 && y == Integer.MIN_VALUE)
    ...
    public Integer multiply(Integer x, Integer y) {
        if (flag && ((x > 0 && y > 0 && x > Integer.MAX_VALUE / y)
    ...
    public Integer divide(Integer x, Integer y) {
        ...
        if (flag && x == Integer.MIN_VALUE && y == -1) throw new OverflowExceptions("overflow");
        return x / y;
    }

    @Override
    public Integer negate(Integer x) {
        if (flag && x == Integer.MIN_VALUE) throw new OverflowExceptions("overflow");
        return -x;
    }
    ...
    public Integer square(Integer x) {
        if (abs(x) > 46341 && flag) throw new OverflowExceptions("overflow");
        return x * x;
    }
